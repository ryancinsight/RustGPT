...
fn main() {
    ...
    println!(
        ...
        poly_sigmoid.gate_poly.weights().len()
    );
    println!(
        "  None gate parameters: {}",
        poly_none.gate_poly.weights().len()
    );
    println!();
    ...
    println!(
        "  Sigmoid output - mean: {:.6}, std: {:.6}",
        sigmoid_mean, sigmoid_std
    );
    println!(
        "  None output - mean: {:.6}, std: {:.6}",
        none_mean, none_std
    );
    println!();
    ...
    for epoch in 0..epochs {
        ...
        if epoch % 10 == 0 || epoch == epochs - 1 {
            println!(
                ...
                epoch,
                poly_sigmoid.gate_poly.weights()
            );
            println!(
                "  Epoch {}: None params: {:?}",
                epoch,
                poly_none.gate_poly.weights()
            );
            println!();
        }
    }
    ...
    println!(
        "  ✅ None variant has {} more learnable parameters",
        none_final.len() - sigmoid_final.len()
    );
    ...
    if none_max_change > sigmoid_max_change {
        println!(
            "  ✅ None variant shows greater parameter adaptation ({:.6} vs {:.6})",
