fn main() {
    ...
    // Fixed parameters: nu=1.0, k=1.0 (changed from 2.0), m=0.0, beta=1.0, a=1.0, b=0.0, scale=1.0,
    // shift=0.0
    ...
    let test_inputs = vec![
        0.1f64, 0.5, 1.0, 1.5, 2.0, 3.0, 5.0, 10.0, -0.1, -0.5, -1.0, -1.5, -2.0, -3.0, -5.0, -10.0,
    ];
    ...
    for &x in &test_inputs {
        ...
        println!(
            "x={:6.1}: Richards={:10.6}, StdTanh={:10.6}, AbsDiff={:10.6}, RelDiff={:8.4}%",
            x,
            richards_output,
            std_tanh_output,
            diff,
    ...
    if max_diff < 1e-10 {
        println!(
            "✅ SUCCESS: Fixed RichardsCurve::tanh now matches standard tanh with machine precision!"
        );
        ...
        println!(
            "❌ ISSUE: Fixed RichardsCurve::tanh still has significant differences from standard tanh"
        );
    }
    ...
    for &x in &test_inputs {
        ...
        println!(
            "x={:6.1}: RichardsGrad={:10.6}, StdTanhGrad={:10.6}, AbsDiff={:10.6}, RelDiff={:8.4}%",
            x,
            richards_grad,
            std_tanh_grad,
            grad_diff,
    ...
    println!(
        ...
        max_grad_rel_diff * 100.0
    );

    // Check gradient fix
    if max_grad_diff < 1e-10 {
        println!(
            "✅ SUCCESS: Fixed RichardsCurve gradients now match standard tanh gradients with machine precision!"
        );
        ...
        println!(
            "❌ ISSUE: Fixed RichardsCurve gradients still have significant differences from standard tanh gradients"
        );
    }
    ...
    println!(
        "Gradient norm ratio (Richards/Standard): {:.6}",
        grad_norm_ratio
    );
    ...
    if (grad_norm_ratio - 1.0).abs() < 0.01 {
        ...
        println!(
            "⚠️  WARNING: Richards curve produces {:.1}% higher gradient norms!",
            (grad_norm_ratio - 1.0) * 100.0
        );
    } else if grad_norm_ratio < 0.9 {
        println!(
            "ℹ️  INFO: Richards curve produces {:.1}% lower gradient norms",
    ...
    for &x in &[0.5f64, 1.0, 2.0, -0.5, -1.0, -2.0] {
        ...
        println!(
            "x={:4.1}: Original={:8.5} (diff={:8.5}), Fixed={:8.5} (diff={:8.5}), StdTanh={:8.5}",
