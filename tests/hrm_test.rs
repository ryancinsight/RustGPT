...
fn test_hrm_parameter_count_vs_transformer() {
    ...
    assert!(
        ratio > 0.5 && ratio < 1.5,
        "Parameter count ratio should be between 0.5 and 1.5"
    );
}
...
fn test_hrm_hierarchical_convergence() {
    ...
    assert!(
        diff < 1e-4,
        "Outputs should be nearly identical for same input"
    );
}
...
fn test_hrm_serialization() {
    ...
    let hrm_deserialized: HRMBlock =
        serde_json::from_str(&json).expect("Failed to deserialize HRM");

    // Check parameters match
    assert_eq!(hrm.parameters(), hrm_deserialized.parameters());
    assert_eq!(hrm.num_high_cycles(), hrm_deserialized.num_high_cycles());
    assert_eq!(
        hrm.low_steps_per_cycle(),
        hrm_deserialized.low_steps_per_cycle()
    );
}
