...
fn test_rope_rotation_properties() {
    ...
    for pos in 0..5 {
        ...
        assert!(
            (input_mag - output_mag).abs() < 1e-3,
            "Magnitude not preserved at position {}: {} vs {}",
            pos,
            input_mag,
            output_mag
        );
...
fn test_rope_frequency_bands() {
    ...
    assert!(
        rotation1 > rotation2,
        "Lower dimensions should rotate faster: {} vs {}",
        rotation1,
        rotation2
    );
}
...
fn test_rope_integration_with_attention() {
    ...
    let diff: f32 = scores_without_rope
        .iter()
