...
fn test_moh_gating_basic_correctness() {
    ...
    let batch_size = 10;
    let input = Array2::<f32>::from_shape_fn((batch_size, embed_dim), |_| rand::rng().random::<f32>());
...
fn test_moh_gating_constraints() {
    ...
    for i in 0..batch_size {
        ...
        assert!(active_count >= 2, "Token {} has {} active heads, expected >= 2", i, active_count);
        assert!(active_count <= 4, "Token {} has {} active heads, expected <= 4", i, active_count);
        assert!(row[0] > 0.0, "Always-on head 0 is not active for token {}", i);
    }
}
