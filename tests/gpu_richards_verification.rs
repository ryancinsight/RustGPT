#[cfg(feature = "wgpu")]
mod tests {
    use llm::domain::{compute::gpu_device::GpuDevice, network::Layer, richards::RichardsGlu};
    use ndarray::Array2;
    use rand::Rng;
    use std::sync::{Arc, Mutex};

    #[tokio::test]
    async fn test_richards_glu_gpu_cpu_parity() {
        // 1. Initialize RichardsGlu
        ...
        // Note: The first run might be slower due to pipeline compilation
        println!("Running GPU forward pass...");
        let output_gpu = layer.forward(&input);

        // 7. Compare results
        // We use a relatively loose epsilon because:
        // - CPU uses f32 (IEEE 754)
        // - GPU uses f32 (but might have different rounding/fused instructions)
        // - Richards Curve involves exp() and division which can accumulate errors
        let epsilon = 1e-4;
