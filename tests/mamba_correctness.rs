...
fn test_mamba_forward_backward_shapes() {
    ...
    assert!(!output.iter().any(|x: &f32| x.is_nan()), "Output contains NaN");
    ...
    assert!(!input_grads.iter().any(|x: &f32| x.is_nan()), "Input grads contain NaN");
}
...
fn test_mamba_numerical_stability() {
    ...
    let input = Array2::<f32>::zeros((seq_len, d_model)).mapv(|_| (rand::random::<f32>() - 0.5) * 2.0);
    
    let output = mamba.forward(&input);
    
    // Check if output exploded
    let max_val = output.iter().fold(0.0f32, |a: f32, &b: &f32| a.max(b.abs()));
    ...
    let max_grad = input_grads.iter().fold(0.0f32, |a: f32, &b: &f32| a.max(b.abs()));
