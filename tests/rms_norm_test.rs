...
fn test_rms_norm_no_mean_centering() {
    ...
    assert!(
        ...
        "RMSNorm should not center by mean, got mean={}",
        output_mean
    );
}
...
fn test_rms_norm_gradient_flow() {
    ...
    assert!(
        ...
        "Gamma gradients should not be all zeros"
    );
}
...
fn test_rms_norm_numerical_stability() {
    ...
    let mixed_input =
        Array2::from_shape_vec((2, 4), vec![1e-8, 1e8, -1e-8, -1e8, 0.0, 1.0, -1.0, 100.0])
            .unwrap();
...
fn test_rms_norm_custom_epsilon() {
    ...
    assert!(
        output.iter().all(|&x| x.is_finite()),
        "Output should be finite with custom epsilon"
    );
}
...
fn test_rms_norm_batch_independence() {
    ...
    for i in 0..4 {
        ...
        assert!(
            (batch_val1 - single_val1).abs() < 1e-5,
            "Batch and single processing should match for token 1, feature {}",
            i
        );
        ...
        assert!(
            (batch_val2 - single_val2).abs() < 1e-5,
            "Batch and single processing should match for token 2, feature {}",
            i
        );
    }
}
