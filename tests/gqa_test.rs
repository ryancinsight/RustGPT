...
fn test_gqa_creation() {
    let attention = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 4, false, 512, None);
...
fn test_mha_backward_compatibility() {
    let attention_mha = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 8, false, 512, None);
    assert_eq!(attention_mha.num_heads, 8);
    assert_eq!(attention_mha.num_kv_heads, 8);
    let attention_standard = SelfAttention::new_with_heads(EMBEDDING_DIM, 8);
...
fn test_mqa_extreme_case() {
    let attention = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 1, false, 512, None);
    assert_eq!(attention.num_heads, 8);
...
fn test_gqa_invalid_grouping() {
    SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 3, false, 512, None);
}

#[test]
#[should_panic(expected = "num_heads must be divisible by num_kv_heads")]
fn test_gqa_invalid_kv_heads() {
    SelfAttention::new_with_gqa(EMBEDDING_DIM, 4, 8, false, 512, None);
}

#[test]
fn test_gqa_parameter_reduction() {
    let head_dim = EMBEDDING_DIM / 8;
    let mha = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 8, false, 512, None);
    let mha_params = mha.parameters();
    let expected_mha = 8 * head_dim * head_dim + 8 * 2 * head_dim * head_dim;
    assert_eq!(mha_params, expected_mha);
    let gqa = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 4, false, 512, None);
    ...
    let mqa = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 1, false, 512, None);
...
fn test_gqa_forward_pass() {
    let mut gqa = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 4, false, 512, None);
...
fn test_gqa_with_rope() {
    let mut gqa = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 4, true, 512, None);
    let input = Array2::ones((5, EMBEDDING_DIM));
    let output = gqa.forward(&input);
    assert_eq!(output.shape(), [5, EMBEDDING_DIM]);
    assert!(output.iter().any(|&x| x != 0.0));
...
fn test_gqa_vs_mha_output_similarity() {
    let mut mha = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 8, false, 512, None);
    let mut gqa = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 4, false, 512, None);
...
fn test_gqa_kv_cache_size_reduction() {
    let head_dim = EMBEDDING_DIM / 8;
    let _mha = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 8, false, 512, None);
    let mha_kv_params = 8 * 2 * head_dim * head_dim;
    let _gqa = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 4, false, 512, None);
    let gqa_kv_params = 4 * 2 * head_dim * head_dim;
    assert_eq!(mha_kv_params, gqa_kv_params * 2);
    let _mqa = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 1, false, 512, None);
...
fn test_gqa_with_rope_integration() {
    let mut gqa_with_rope = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 4, true, 512, None);
    let mut gqa_without_rope = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 4, false, 512, None);
    let input = Array2::from_elem((5, EMBEDDING_DIM), 0.1);
    ...
    let diff: f32 = output_with_rope.iter().zip(output_without_rope.iter()).map(|(a, b)| (a - b).abs()).sum();
    assert!(diff > 0.0, "RoPE should change the output");
}

#[test]
fn test_gqa_grouping_correctness() {
    let gqa_4kv = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 4, false, 512, None);
    assert_eq!(gqa_4kv.num_heads / gqa_4kv.num_kv_heads, 2);
    let gqa_2kv = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, 2, false, 512, None);
...
fn test_gqa_parameter_count_consistency() {
    ...
    for num_kv_heads in [1, 2, 4, 8] {
        let gqa = SelfAttention::new_with_gqa(EMBEDDING_DIM, 8, num_kv_heads, false, 512, None);
        let expected_params = 8 * head_dim * head_dim + num_kv_heads * 2 * head_dim * head_dim;
        assert_eq!(gqa.parameters(), expected_params, "Parameter count mismatch for num_kv_heads={}", num_kv_heads);
    }
}
