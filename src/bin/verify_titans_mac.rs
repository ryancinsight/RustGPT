...
fn main() {
    ...
    // Head 3 is index 3
    let scores_batch_h3 = &batch_data.poly_caches[0].scores_dump.as_ref().unwrap()[3];
    
    let scores_stream_h3 = captured_stream_scores_last_head.as_ref().expect("No stream scores captured");

    println!("\n--- Head 3 (Last Head) Debug ---");
    println!("Batch Scores: {:?}", scores_batch_h3);
    println!("Stream Scores: {:?}", scores_stream_h3);
    println!("Diff: {:.4e}", (scores_batch_h3 - scores_stream_h3).abs().sum());
    ...
    assert!((scores_batch_h3 - scores_stream_h3).abs().sum() < 1e-4, "Head 3 Scores Divergence");
