...
/// Richards-based Normalization (replaces DynamicTanhNorm)
///
...
///
/// Provides a lightweight normalization alternative without batch statistics,
/// with adaptive scaling via Richards' learnable parameters.
...
pub struct RichardsNorm {
    ...
    /// Richards curve for tanh-like computation with learnable parameters
    richards: RichardsCurve,

    /// Optimizers for gamma/beta parameters
...
impl Layer for RichardsNorm {
    ...
    fn compute_gradients(
        ...
        let mut grad_temperature_scalar = 0.0f64;
        ...
        ndarray::Zip::indexed(&mut grad_input)
            ...
            .for_each(|(_, j), o, &x, &dy, &g| {
                ...
                *o = (g * dy) * dt_dx; // grad w.r.t. input
                ...
                grad_temperature_scalar += richards_grads[3];
                grad_scale_scalar += richards_grads[4];
            });
        ...
        let grad_temperature = Array2::from_shape_vec((1, 1), vec![grad_temperature_scalar as f32]).unwrap();
        let grad_scale = Array2::from_shape_vec((1, 1), vec![grad_scale_scalar as f32]).unwrap();
        (grad_input, vec![grad_nu, grad_k, grad_beta_richards, grad_temperature, grad_scale, grad_gamma, grad_beta])
    }
    ...
    fn apply_gradients(
        ...
        if param_grads.len() != 7 {
            return Err(crate::errors::ModelError::GradientError {
                message: format!(
                    "RichardsNorm expected 7 parameter gradients (nu, k, beta_richards, temperature, scale, gamma, beta), got {}",
                    param_grads.len()
                ),
            });
        }
        self.richards.step(&[
            ...
            param_grads[3][[0, 0]] as f64, // temperature
            param_grads[4][[0, 0]] as f64, // scale
        ], lr as f64);
        self.optimizer_gamma
            .step(&mut self.gamma, &param_grads[5], lr);
        self.optimizer_beta
            .step(&mut self.beta, &param_grads[6], lr);
        Ok(())
    }
