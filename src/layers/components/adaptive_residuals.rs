...
use crate::{adam::Adam, errors::Result};
...
impl AdaptiveResidualStrategy for UnifiedAdaptiveResiduals {
    fn apply_attention_residual(
        ...
        for seq_idx in 0..seq_len {
            ...
            for embed_idx in 0..self.embed_dim {
                let affinity = smooth_clamp(
                    self.layer_affinity_scores[[embed_idx, 0]],
                    0.0,
                    2.0,
                    beta_clamp,
                );
                ...
                let input_safe = if input_val.is_finite() {
                    input_val
                } else {
                    0.0
                };
    ...
    fn apply_ffn_residual(&self, residual1: &Array2<f32>, ffn_out: &Array2<f32>) -> Array2<f32> {
        ...
        for seq_idx in 0..seq_len {
            ...
            for embed_idx in 0..self.embed_dim {
                ...
                let residual_safe = if residual_val.is_finite() {
                    residual_val
                } else {
                    0.0
                };
                let ffn_safe = if ffn_val.is_finite() { ffn_val } else { 0.0 };
                let scale_safe = if effective_scale.is_finite() {
                    effective_scale
                } else {
                    1.0
                };
    ...
    fn compute_gradients(
        ...
        for embed_idx in 0..embed_dim {
            let residual_sum: f32 = (0..seq_len)
                .map(|seq| residual_grads[[seq, embed_idx]].abs())
                .sum();
        ...
        for embed_idx in 0..embed_dim {
            let attn_residual_sum: f32 = (0..seq_len)
                .map(|seq| residual_grads[[seq, embed_idx]] * attn_out[[seq, embed_idx]])
                .sum();
            attention_scale_grads[[embed_idx, 0]] = attn_residual_sum * 0.001;

            let ffn_residual_sum: f32 = (0..seq_len)
                .map(|seq| residual_grads[[seq, embed_idx]] * ffn_out[[seq, embed_idx]])
                .sum();
    ...
    fn apply_gradients(&mut self, param_grads: &[Array2<f32>], lr: f32) -> Result<()> {
        if param_grads.len() < 9 {
            return Err(crate::errors::ModelError::GradientError {
                message: format!(
                    "Expected at least 9 gradient arrays, got {}",
                    param_grads.len()
                ),
            });
        }
        ...
        self.opt_similarity
            .step(&mut self.weight_similarity_matrix, similarity_grads, lr);
        let norm = self
            .weight_similarity_matrix
            .iter()
            .map(|x| x * x)
        ...
        self.opt_affinity
            .step(&mut self.layer_affinity_scores, affinity_grads, lr);
        self.layer_affinity_scores
            .mapv_inplace(|x| smooth_clamp(x, -2.0, 2.0, 10.0));
        idx += 1;

        // Attention parameters
        let attention_grads = &param_grads[idx];
        self.opt_attention
            .step(&mut self.residual_attention_qkv, attention_grads, lr);
        idx += 1;

        // Channel affinity
        let channel_grads = &param_grads[idx];
        self.opt_channel
            .step(&mut self.channel_affinity_matrix, channel_grads, lr);
        for i in 0..self.embed_dim {
            for j in 0..i {
                let avg = (self.channel_affinity_matrix[[i, j]]
                    + self.channel_affinity_matrix[[j, i]])
                    * 0.5;
        ...
        self.opt_scales_attention.step(
            ...
            attention_scale_grads,
            lr,
        );
        self.attention_residual_scales
            .mapv_inplace(|x| smooth_clip_tanh(x, self.residual_stability_threshold));
        idx += 1;

        // FFN scales
        let ffn_scale_grads = &param_grads[idx];
        self.opt_scales_ffn
            .step(&mut self.ffn_residual_scales, ffn_scale_grads, lr);
        ...
        self.opt_positional_qkv
            .step(&mut self.positional_residual_qkv, positional_qkv_grads, lr);
        idx += 1;

        let cope_pos_grads = &param_grads[idx];
        self.opt_cope_pos
            .step(&mut self.cope_pos_embeddings, cope_pos_grads, lr);
        idx += 1;

        let positional_weights_grads = &param_grads[idx];
        self.opt_positional_weights.step(
            &mut self.positional_residual_weights,
            positional_weights_grads,
            lr,
        );
    ...
    fn get_performance_metrics(&self) -> (f32, f32, f32) {
        ...
        let affinity_entropy = -self
            ...
            .map(|&x| {
                ...
                let p = curve.forward_scalar(x as f64) as f32;
        ...
        let similarity_variance = self
            .weight_similarity_matrix
            .iter()
            .map(|x| (x - mean_similarity).powi(2))
            .sum::<f32>()
            / self.weight_similarity_matrix.len() as f32;

        let scale_stability = (self
            .attention_residual_scales
            .iter()
            .chain(self.ffn_residual_scales.iter()))
        .map(|x| x.abs())
        .sum::<f32>()
            / (2 * self.embed_dim) as f32;

        (
            affinity_entropy,
            similarity_variance.sqrt(),
            scale_stability,
        )
    }
}
...
mod tests {
    ...
    fn test_adaptive_residual_trait() {
        ...
        let mut residuals: Box<dyn AdaptiveResidualStrategy> =
            Box::new(UnifiedAdaptiveResiduals::new(embed_dim));
    ...
    fn test_similarity_matrix_consistency() {
        ...
        let diff = (&original_values - &residuals.weight_similarity_matrix)
            .mapv(|x| x.abs())
            .mean()
            .unwrap_or(1.0);
        assert!(
