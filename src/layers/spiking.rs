...
pub struct LifLayer {
    dim: usize,
    config: crate::eprop::NeuronConfig,
...
impl LifLayer {
    pub fn new(dim: usize) -> Self {
        let mut config = crate::eprop::NeuronConfig::lif();
...
impl Layer for LifLayer {
    ...
    fn apply_gradients(
        ...
        if gradients.is_empty() {
            ...
            Err(crate::errors::ModelError::GradientError {
...
impl AlifLayer {
    pub fn new(dim: usize) -> Self {
        let mut config = crate::eprop::NeuronConfig::alif();
        config.use_adaptive_surrogate = false;
        Self {
            dim,
            config,
