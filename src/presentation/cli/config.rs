...
pub fn build_model_config(args: &Args) -> ModelConfig {
    ...
    config.moh_threshold_modulation = if args.moh_threshold_modulation_adaptive {
        let mut curve = crate::domain::richards::RichardsCurve::new_default();
        curve.m = Some(args.moh_threshold_modulation_curve_m as f64);
        curve.k = Some(args.moh_threshold_modulation_curve_k as f64);
        crate::domain::richards::adaptive::AdaptiveScalar::Richards {
            curve: Box::new(curve),
            output_scale: args.moh_threshold_modulation,
        }
    } else {
        crate::domain::richards::adaptive::AdaptiveScalar::Fixed(args.moh_threshold_modulation)
    };
    ...
    if args.moe {
        config.moe_router = Some(crate::domain::mixtures::moe::ExpertRouter::LearnedMoE {
            num_experts: 4,
