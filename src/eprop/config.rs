...
impl Default for NeuronConfig {
    fn default() -> Self {
        Self {
            ...
            initial_surrogate_function: super::adaptive_surrogate::SurrogateFunction::PiecewiseLinear,
    ...
    pub fn validate(&self) -> super::Result<()> {
        if self.alpha <= 0.0 || self.alpha >= 1.0 {
            return Err(super::EPropError::InvalidConfig(
                format!("alpha must be in (0, 1), got {}", self.alpha)
            ));
        }
        
        if self.v_threshold <= 0.0 {
            return Err(super::EPropError::InvalidConfig(
                format!("v_threshold must be positive, got {}", self.v_threshold)
            ));
        }
        ...
        if self.model == NeuronModel::ALIF {
            if self.rho <= 0.0 || self.rho >= 1.0 {
                return Err(super::EPropError::InvalidConfig(
                    format!("rho must be in (0, 1), got {}", self.rho)
                ));
            }
            
            if self.beta < 0.0 {
                return Err(super::EPropError::InvalidConfig(
                    format!("beta must be non-negative, got {}", self.beta)
                ));
            }
        }
        ...
        if self.gamma_pd <= 0.0 {
            return Err(super::EPropError::InvalidConfig(
                format!("gamma_pd must be positive, got {}", self.gamma_pd)
            ));
        }
        
        if self.surrogate_adaptation_rate <= 0.0 || self.surrogate_adaptation_rate > 1.0 {
            return Err(super::EPropError::InvalidConfig(
                format!("surrogate_adaptation_rate must be in (0, 1], got {}", self.surrogate_adaptation_rate)
            ));
        }
...
impl Default for EPropConfig {
    fn default() -> Self {
        Self {
            ...
            use_checkpointing: true,  // Enable for all training (auto-thresholds for short sequences)
...
impl EPropConfig {
    ...
    pub fn validate(&self) -> super::Result<()> {
        ...
        if self.alpha_smooth <= 0.0 || self.alpha_smooth >= 1.0 {
            return Err(super::EPropError::InvalidConfig(
                format!("alpha_smooth must be in (0, 1), got {}", self.alpha_smooth)
            ));
        }
        
        if self.learning_rate <= 0.0 {
            return Err(super::EPropError::InvalidConfig(
                format!("learning_rate must be positive, got {}", self.learning_rate)
            ));
        }
        ...
        if let Some(clip) = self.grad_clip {
            if clip <= 0.0 {
                return Err(super::EPropError::InvalidConfig(
                    format!("grad_clip must be positive, got {}", clip)
                ));
            }
        }
        ...
        if self.init_scale <= 0.0 {
            return Err(super::EPropError::InvalidConfig(
                format!("init_scale must be positive, got {}", self.init_scale)
            ));
        }
    ...
    pub fn with_vocab_optimization(
        ...
        if matches!(self.softmax_strategy, Some(SoftmaxStrategy::Sampled)) {
            self.num_negative_samples = ((vocab_size as f32).sqrt() as usize)
                .min(5_000)
                .max(100);
        }
