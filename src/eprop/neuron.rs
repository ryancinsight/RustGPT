...
use ndarray::Array1;
use crate::eprop::config::{NeuronConfig, NeuronModel};
// use crate::eprop::adaptive_surrogate::{AdaptiveSurrogate, SurrogatePerformance, ActivityStats};
use crate::eprop::adaptive_surrogate::SurrogatePerformance;
...
impl NeuronState {
    ...
    pub fn new(num_neurons: usize, use_adaptation: bool, config: &NeuronConfig) -> Self {
        let performance = if config.use_adaptive_surrogate {
            Some(SurrogatePerformance::new(config.surrogate_performance_window))
        } else {
            None
        };
...
impl NeuronDynamics {
        ...
        if self.config.use_adaptive_surrogate {
            if let Some(ref mut performance) = state.performance {
                ...
                if let Some(ref loss_grad) = loss_gradient {
                    let _ = performance.update_with_gradient(
                        loss_grad,
                        &surrogate_deriv,
                        current_loss
                    );
    ...
    fn compute_adaptive_spikes(
        ...
        let perf = state.performance.as_mut()
    ...
    fn compute_spikes(&self, voltage: &Array1<f32>, threshold: &Array1<f32>) -> (Array1<f32>, Array1<f32>) {
        let n = voltage.len();
        let mut spikes = Array1::zeros(n);
        let mut surrogate_deriv = Array1::zeros(n);
