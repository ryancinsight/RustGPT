...
// - src/model/config.rs (compiled as `crate::model_config`)
// - and also available under `crate::model::config`.
//
// This file is intentionally empty so it cannot diverge.

compile_error!("src/model_config.rs is legacy; use src/model/config.rs");
