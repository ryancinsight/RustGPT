...
impl WindowAwareCoPE {
    ...
    pub fn new_standard(max_pos: usize, embed_dim: usize, window_size: Option<usize>) -> Self {
        ...
        let pos_embeddings = Array2::<f32>::from_shape_fn((max_pos + 1, embed_dim), |_| normal_pe.sample(&mut rng));
    ...
    pub fn new_optimized(max_pos: usize, embed_dim: usize, rank: usize, window_size: Option<usize>) -> Self {
        use rand_distr::Distribution;
        use crate::common::rng::get_rng;
    ...
    pub fn new_gated(max_pos: usize, embed_dim: usize, window_size: Option<usize>) -> Self {
        ...
        // Base CoPE
        let pos_embeddings = Array2::<f32>::from_shape_fn((max_pos + 1, embed_dim), |_| normal.sample(&mut rng));
        
        // Gate projection: (2*embed_dim, max_pos + 1)
        let w_gate = Array2::from_shape_fn((2 * embed_dim, max_pos + 1), |_| normal.sample(&mut rng) * 0.01);
    ...
    pub fn new_factorized(max_pos: usize, embed_dim: usize, rank: usize, window_size: Option<usize>) -> Self {
        use rand_distr::Distribution;
        use crate::common::rng::get_rng;
...
impl WindowAwareCoPE {
    ...
    pub fn cope_contribution(&self, q: &ArrayView1<'_, f32>, pos: usize) -> f32 {
        ...
        match &self.cope_type {
            ...
            WindowAwareCoPEType::Optimized => {
                self.optimized_cope_contribution(q, q, pos)
            }
            WindowAwareCoPEType::Gated => {
                if let (Some(pe), Some(w), Some(b)) = (&self.pos_embeddings, &self.w_gate, &self.b_gate) {
                    if eff_pos > self.max_pos { return 0.0; }
            ...
            WindowAwareCoPEType::Factorized => {
                self.factorized_cope_contribution(q, pos)
            }
        }
    }
    ...
    pub fn optimized_cope_contribution(
        ...
        let (up, down, w_gate, b_gate) = match (&self.up_proj, &self.down_proj, &self.w_gate, &self.b_gate) {
    ...
    pub fn parameters(&self) -> usize {
        match self.cope_type {
            WindowAwareCoPEType::Standard => {
                self.pos_embeddings.as_ref().map_or(0, |pe| pe.len())
            }
            WindowAwareCoPEType::Optimized => {
...
mod tests {
    ...
    fn test_dynamic_window_resize() {
        ...
        assert!(cope.cope_contribution(&q.view(), 15).is_finite(), "pos 15 should be in window");
        assert_eq!(cope.cope_contribution(&q.view(), 16), 0.0, "pos 16 should be out of window");
        ...
        assert!(cope.cope_contribution(&q.view(), 31).is_finite(), "pos 31 should be in window");
        assert_eq!(cope.cope_contribution(&q.view(), 32), 0.0, "pos 32 should be out of window");
