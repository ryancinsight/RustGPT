...
impl QuantizedEligibilityTraces {
    ...
    pub fn new(input_dim: usize, num_neurons: usize, use_adaptation: bool, scale: f32) -> Self {
        ...
        let eps_a_q = if use_adaptation {
            Some(vec![0; num_neurons])
        } else {
            None
        };
        ...
        let eps_a_fp = if use_adaptation {
    ...
    pub fn get_full_precision_traces(
        &mut self,
    ) -> (&Array1<f32>, &Array1<f32>, Option<&Array1<f32>>) {
    ...
    pub fn memory_savings(&self) -> (usize, f32) {
        ...
        let full_precision_only = (self.eps_x_q.len()
            + self.eps_f_q.len()
            + self.eps_a_q.as_ref().map_or(0, |v| v.len()))
            * 4;
