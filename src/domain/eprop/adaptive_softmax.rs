...
//!
//! ```rust
//! use llm::domain::eprop::adaptive_softmax::{AdaptiveSoftmax, SoftmaxConfig};
//! use ndarray::Array1;
//!
//! // Automatic strategy selection
//! let word_frequencies = vec![1.0; 50_000];
...
impl AdaptiveSoftmax {
    ...
    pub fn new(config: SoftmaxConfig) -> Self {
        ...
        let (sampled, hierarchical) = match strategy {
            SoftmaxStrategy::Sampled => (Some(SampledSoftmaxImpl::new(&config)), None),
            ...
            SoftmaxStrategy::Hierarchical => (None, Some(HierarchicalSoftmaxImpl::new(&config))),
    ...
    pub fn forward(&self, logits: &Array1<f32>) -> Array1<f32> {
        ...
        match self.strategy {
            SoftmaxStrategy::Full | SoftmaxStrategy::Sampled => self.full_softmax_forward(logits),
            SoftmaxStrategy::Hierarchical => self.hierarchical.as_ref().unwrap().forward(logits),
...
impl SampledSoftmaxImpl {
    fn new(config: &SoftmaxConfig) -> Self {
        ...
        let unigram_dist = if let Some(ref freqs) = config.frequencies {
            freqs
                .iter()
                .map(|&f| f.powf(config.unigram_power))
                .collect()
        } else {
        ...
        let rng = if let Some(seed) = config.seed.or_else(crate::common::rng::get_seed) {
    ...
    fn sample_negatives(&mut self, target: usize, num_samples: usize) {
        ...
        while self.sample_set.len() < num_samples.min(self.vocab_size) + 1 {
            ...
            let idx = match self
                .cumulative_dist
                .binary_search_by(|&p| p.partial_cmp(&r).unwrap_or(std::cmp::Ordering::Equal))
            {
                Ok(i) => i,
...
impl HierarchicalSoftmaxImpl {
    ...
    fn build_balanced(
        leaves: &[Node],
        internal_nodes: &mut Vec<(Node, Node)>,
        next_node_idx: &mut usize,
    ) -> Node {
        if leaves.len() == 1 {
            return leaves[0];
        }
