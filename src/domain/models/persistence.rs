...
use crate::{
    ...
    domain::{models::llm::LLM, network::LayerEnum},
};
...
impl LLM {
    ...
    fn get_architecture_name(&self) -> String {
        ...
        let has_transformer = self
            .network
            .iter()
            .any(|l| matches!(l, LayerEnum::TransformerBlock(_)));
        ...
        for layer in &self.network {
            let tm = match layer {
                LayerEnum::TransformerBlock(tb) => match &tb.temporal_mixing().temporal_mixing {
                    ...
                    crate::domain::layers::components::common::TemporalMixingLayer::RgLru(_) => {
                        Some(TM::RgLru)
                    }
                    crate::domain::layers::components::common::TemporalMixingLayer::Mamba(_) => {
                        Some(TM::Mamba)
                    }
    ...
    fn get_embedding_dim(&self) -> usize {
        ...
        for layer in &self.network {
            if let LayerEnum::TokenEmbeddings(emb) = layer {
