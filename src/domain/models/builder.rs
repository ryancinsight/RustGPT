...
fn build_diffusion_layers(
    ...
    layers.push(LayerEnum::DynamicTanhNorm(Box::new(
        crate::domain::richards::RichardsNorm::new(config.embedding_dim),
    )));
}
...
pub fn print_architecture_summary(config: &ModelConfig, layers: &[LayerEnum]) {
    ...
    if matches!(
        ...
        crate::domain::models::config::TemporalMixingType::Attention
    ) {
        println!("\n🧠 Attention:");
        use crate::domain::models::config::AttentionType;
