// Legacy file retained for compatibility with older checkouts.
//
// The implementation has moved to src/model/persistence.rs and is internal.
// Public API is `LLM::{save_versioned, load_versioned}`.
//
// This file is intentionally empty so it cannot diverge.

compile_error!("src/model_persistence.rs is legacy; use src/model/persistence.rs via LLM methods");
