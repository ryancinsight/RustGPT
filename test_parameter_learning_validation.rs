...
fn main() {
    ...
    println!(
        "  Sigmoid learnable (a,b fixed):     {} parameters",
        sigmoid_learnable.weights().len()
    );
    println!(
        "  Sigmoid fixed (all fixed):         {} parameters",
        sigmoid_fixed.weights().len()
    );
    println!(
        "  None variant (all learnable):      {} parameters",
        none_variant.weights().len()
    );
    println!(
        "  Tanh learnable (a,b fixed):        {} parameters",
        tanh_learnable.weights().len()
    );
    println!(
        "  Gompertz learnable (a,b fixed):    {} parameters",
        gompertz_learnable.weights().len()
    );

    assert_eq!(
        sigmoid_learnable.weights().len(),
        6,
        "Sigmoid learnable should have 6 parameters"
    );
    assert_eq!(
        sigmoid_fixed.weights().len(),
        0,
        "Sigmoid fixed should have 0 parameters"
    );
    assert_eq!(
        none_variant.weights().len(),
        8,
        "None variant should have 8 parameters"
    );
    assert_eq!(
        tanh_learnable.weights().len(),
        6,
        "Tanh learnable should have 6 parameters"
    );
    assert_eq!(
        gompertz_learnable.weights().len(),
        6,
        "Gompertz learnable should have 6 parameters"
    );
    ...
    println!(
        "  Sigmoid learnable - output_gain: {:?}, output_bias: {:?}",
        sigmoid_learnable.output_gain, sigmoid_learnable.output_bias
    );
    println!(
        "  None variant - output_gain: {:?}, output_bias: {:?}",
        none_variant.output_gain, none_variant.output_bias
    );
    println!(
        "  Tanh learnable - output_gain: {:?}, output_bias: {:?}",
        tanh_learnable.output_gain, tanh_learnable.output_bias
    );

    assert_eq!(
        sigmoid_learnable.output_gain,
        Some(1.0),
        "Sigmoid should have output_gain=1.0 fixed"
    );
    assert_eq!(
        sigmoid_learnable.output_bias,
        Some(0.0),
        "Sigmoid should have output_bias=0.0 fixed"
    );
    assert_eq!(
        none_variant.output_gain, None,
        "None variant should have output_gain=None (learnable)"
    );
    assert_eq!(
        none_variant.output_bias, None,
        "None variant should have output_bias=None (learnable)"
    );
    assert_eq!(
        tanh_learnable.output_gain,
        Some(1.0),
        "Tanh should have output_gain=1.0 fixed"
    );
    assert_eq!(
        tanh_learnable.output_bias,
        Some(0.0),
        "Tanh should have output_bias=0.0 fixed"
    );
    ...
    assert!(
        params_changed,
        "Parameters should have changed during learning"
    );
    ...
    println!(
        "  Sigmoid curve - Initial a,b: {:.3}, {:.3} -> Final a,b: {:.3}, {:.3}",
        sigmoid_initial[4], sigmoid_initial[5], sigmoid_final[4], sigmoid_final[5]
    );
    println!(
        "  None curve - Initial a,b: {:.3}, {:.3} -> Final a,b: {:.3}, {:.3}",
        none_initial[4], none_initial[5], none_final[4], none_final[5]
    );

    // Sigmoid should have fixed a,b (no change)
    assert!(
        (sigmoid_initial[4] - sigmoid_final[4]).abs() < 1e-10,
        "Sigmoid a should remain fixed"
    );
    assert!(
        (sigmoid_initial[5] - sigmoid_final[5]).abs() < 1e-10,
        "Sigmoid b should remain fixed"
    );
    ...
    println!(
        "  ✅ None variant a,b coefficients are learnable: a_changed={}, b_changed={}",
